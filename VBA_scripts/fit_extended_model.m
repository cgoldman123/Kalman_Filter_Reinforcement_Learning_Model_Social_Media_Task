function [fit_info, model_output] = fit_extended_model(formatted_file, result_dir, MDP)
    if ispc
        root = 'L:/';
    else
        root = '/media/labs/';
    ...
    dim = struct( ...
        'n', 4, ... number of hidden states (2 expected values, two variances)
        'n_theta', length(MDP.evolution_params), ... number of evolution parameters (1: learning rate)
        'n_phi', length(MDP.observation_params) ... number of observation parameters (1: temperature)
       );
